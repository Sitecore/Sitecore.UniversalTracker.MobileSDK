namespace Sitecore.MobileSDK.Session
{
  ...
  internal class SessionBuilder : IAuthenticatedSessionBuilder, IAnonymousSessionBuilder, IEntitySessionBuilder
  {
    ...
    public IBaseSessionBuilder OdataApiKey(string apiKeyItemId){

      if (string.IsNullOrEmpty(apiKeyItemId))
      {
        return this;
      }

      BaseValidator.CheckForTwiceSetAndThrow(this.apiKeyItemId,
        this.GetType().Name + ".apiKeyItemId");
      BaseValidator.CheckForNullEmptyAndWhiteSpaceOrThrow(apiKeyItemId,
        this.GetType().Name + ".apiKeyItemId");

      this.apiKeyItemId = apiKeyItemId;
